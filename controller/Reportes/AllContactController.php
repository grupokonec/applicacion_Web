...
{
    public function getData(string $fecha1, string $fecha2)
    {
        $conn = new ConnectionOne("bdcl22"); // Instanciamos la conexion.

        // Verificamos que la fecha contenga un un valor.
        if (strlen($fecha1) > 0 && strlen($fecha2) > 0) {
            // Realizamos la consulta
            $query = "SELECT
           c.RUT, ac.tipo, ac.respuesta, ac.fecha, ac.feccomp, ac.telefono, ac.glosa,
            c.IDEmpresaCobranza, c.MesAsignacion, c.Segmento, c.IDCampana, c.IDGrupoControl, c.Contrato
        FROM (
            SELECT *
FROM bdcl22.all_contacts
WHERE date(fecha) BETWEEN '$fecha1' AND '$fecha2'
      AND tipo IN ('AU', 'EM', 'SM', 'VI')
        ...
        INNER JOIN (
            ...
            FROM bdcl22.cartera_primer_dia c1
            GROUP BY c1.RUTSD, c1.RUT

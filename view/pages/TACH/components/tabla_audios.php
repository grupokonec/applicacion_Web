...
if (isset($_POST['resultArray'])) {
    ...
    if ($resultArray !== null && json_last_error() === JSON_ERROR_NONE && (is_array($resultArray) || is_object($resultArray))) {
        ...
        if (!empty($resultArray)) {
            ...
            echo '<tr>';
            echo '<th>ruteje</th>';
            echo '<th>nombreeje</th>';
            echo '<th>caso</th>';
            echo '<th>rut_cliente</th>';
            echo '<th>tipificacion</th>';
            echo '<th>incio_call</th>';
            echo '<th>link</th>';
            echo '<th>cuenta_sau</th>';
            echo '</tr>';
            echo '</thead>';
            echo '<tbody>';
            ...
            foreach ($resultArray as $row) {
                echo '<tr>';
                echo '<td>' . htmlspecialchars($row->ruteje) . '</td>';
                echo '<td>' . htmlspecialchars($row->nombreeje) . '</td>';
                echo '<td>' . htmlspecialchars($row->caso) . '</td>';
                echo '<td>' . htmlspecialchars($row->rut_cliente) . '</td>';
                echo '<td>' . htmlspecialchars($row->tipificacion) . '</td>';
                echo '<td>' . htmlspecialchars($row->incio_call) . '</td>';
                echo '<td>' . htmlspecialchars($row->link) . '</td>';
                echo '<td>' . htmlspecialchars($row->cuenta_sau) . '</td>';
                echo '</tr>';
            }
